...
```powershell
.\scripts\write-release-evidence.ps1 -ExpectedVersion 1.3.1 -Tag v1.3.1
```

The evidence file is written next to the package as:

```text
dist\SamhainSecurityNative-1.3.1-win-x64.release-evidence.json
```

Clean-machine evidence is written next to the package as:

```text
dist\SamhainSecurityNative-1.3.1-win-x64.clean-machine-evidence.json
```
...
The `1.3.1` package is stable-channel and integrity-verified, but it remains marked as `unsigned-dev` until a production certificate is available. The package and update manifests keep that status explicit so operator tooling does not mistake it for a signed public installer.
